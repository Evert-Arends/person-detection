...
class PersonDetector(object):
    ...
    def get_localization(self, image, visual=False):
        ...
        with self.detection_graph.as_default():
            ...
            if len(idx_vec) == 0:
                ...
                for idx in idx_vec:
                    ...
                    cv2.circle(image, (xCenter, yCenter), 5, (0, 255, 0), -1)

                    # Add coords to "JSON string"
                    coords_json2 = {'person': count, 'x': xCenter, 'y': yCenter}
                    coords_json.append(dict(coords_json2))
        ...
        print(coords_json)
        _thread.start_new_thread(PersonDetector.post_the_thing, (coords_json, ))
        return self.car_boxes

        # POST

    @staticmethod
    def post_the_thing(coords_json):
        url = 'http://192.168.1.35/'
        requests.post(url, data={"array": coords_json})
